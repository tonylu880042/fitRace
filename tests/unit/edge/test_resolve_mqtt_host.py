...
def test_reachable_explicit_host_is_used_as_is():
    assert (
        resolve_mqtt_host("fitrace-hub.local", 1883, probe=lambda h, p: True)
        == "fitrace-hub.local"
    )


def test_unreachable_explicit_host_never_silently_falls_back_to_localhost():
    ...
    assert resolve_mqtt_host("192.168.0.130", 1883, probe=probe) == "192.168.0.130"
...
def test_unreachable_host_kept_when_no_local_broker():
    ...
    assert (
        resolve_mqtt_host("192.168.0.130", 1883, probe=lambda h, p: False)
        == "192.168.0.130"
    )
